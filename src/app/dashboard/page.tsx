...
export default function AdminDashboard() {
  ...
  // Form expansion state (default to true so features display immediately)
      ...
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div className="bg-slate-900/80 border border-slate-800 rounded-xl p-4 flex justify-between items-center">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-center text-emerald-400 font-bold">+</div>
            <div>
              <h3 className="text-sm font-semibold text-white">List New Digital Product</h3>
              <p className="text-xs text-slate-400">Masterclasses, Courses, eBooks & Templates</p>
            </div>
          </div>
          <button 
            onClick={() => setIsExpanded(!isExpanded)}
            className="bg-slate-800 hover:bg-slate-700 text-slate-200 px-4 py-2 rounded-lg text-xs font-semibold transition-colors cursor-pointer"
          >
            {isExpanded ? 'Collapse View ✕' : 'Expand Form ⚡'}
          </button>
        </div>
